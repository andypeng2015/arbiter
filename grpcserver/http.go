...
//	GET /status/issues — optional JSON issue catalog when the caller supplies one
func NewHTTPServer(addr string, reg *prometheus.Registry) *http.Server {
	return NewHTTPServerWithStatusAndReadinessAndCatalog(addr, reg, nil, nil, nil)
}
...
func NewHTTPServerWithStatus(addr string, reg *prometheus.Registry, status func() any) *http.Server {
	return NewHTTPServerWithStatusAndReadinessAndCatalog(addr, reg, status, nil, nil)
}
...
func NewHTTPServerWithStatusAndReadiness(addr string, reg *prometheus.Registry, status func() any, readiness func() (bool, string)) *http.Server {
	return NewHTTPServerWithStatusAndReadinessAndCatalog(addr, reg, status, readiness, nil)
}

// NewHTTPServerWithStatusAndReadinessAndCatalog also exposes an optional
// machine-readable issue catalog on /status/issues.
func NewHTTPServerWithStatusAndReadinessAndCatalog(addr string, reg *prometheus.Registry, status func() any, readiness func() (bool, string), catalog func() any) *http.Server {
	...
	if catalog != nil {
		mux.HandleFunc("/status/issues", func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusOK)
